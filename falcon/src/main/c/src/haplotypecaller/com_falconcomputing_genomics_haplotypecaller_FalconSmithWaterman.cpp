...
struct SWPairJNIObj init_JNI_obj(JNIEnv* env, jobject& obj){
    ...
    if(JNIObj_mapIter == SWPairJNIObj_map.end()){
        ...
        jclass results_class_global = env->FindClass("Lcom/falconcomputing/genomics/haplotypecaller/FalconSmithWaterman$FalconSWResultsNative;");
        if(results_class_global == NULL){
            throwAccError(env, "Failed to FindClass FalconSWResultsNative");
        }
        objs.results_class = (jclass) env->NewGlobalRef(results_class_global);
 
        jmethodID results_class_cnstrctr = env->GetMethodID(results_class_global, "<init>", "(Lcom/falconcomputing/genomics/haplotypecaller/FalconSmithWaterman;[I[IIII)V");
        if(results_class_cnstrctr == NULL){
            throwAccError(env, "Failed to Get constructor of FalconSWResultsNative");
        }
...
JNIEXPORT void JNICALL Java_com_falconcomputing_genomics_haplotypecaller_FalconSmithWaterman_init_1native(JNIEnv *env, jobject obj){
...
JNIEXPORT void JNICALL Java_com_falconcomputing_genomics_haplotypecaller_FalconSmithWaterman_done_1native(JNIEnv *env, jobject obj){
...
JNIEXPORT jobject JNICALL Java_com_falconcomputing_genomics_haplotypecaller_FalconSmithWaterman_align_1native(JNIEnv *env, jobject thisObj, jbyteArray reference, jbyteArray alternate, jint w_match, jint w_mismatch, jint w_open, jint w_extend, jint overhang_strategy){
